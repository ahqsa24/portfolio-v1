...
const Portfolio = () => {
  ...
  return (
    <div id="portfolio" className="min-h-screen px-4 sm:px-8 md:px-16 lg:px-24 py-12 sm:py-16 md:py-20 lg:py-24">
      ...
      <div className="flex gap-8 mt-12 justify-between bg-gradientMaroon dark:bg-gray-700/50 py-2 rounded-xl hover:bg-gray-600/60 transition-all duration-300 animate-slide-in-up delay-200">
        <div 
          className={`group flex flex-col gap-2 mx-auto items-center py-4 w-[30%] rounded-xl cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-gradientRed/20 ${
            activeTab === 'projects' 
              ? 'bg-gradient-to-r from-gradientRed via-gradientMaroon to-gradientOrange text-amber-50 shadow-lg' 
          ...
          <div className="fade-in w-full animate-slide-in-up delay-300">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
              {getDisplayData(Project, 'projects').map((project, index) => (
                <div key={project.id} className={`group flex flex-col p-4 gap-2 bg-gradientMaroon dark:bg-gray-700/50 rounded-xl cursor-pointer transition-all duration-500 hover:bg-gray-600/70 hover:scale-105 hover:shadow-2xl hover:shadow-gradientRed/20 animate-fade-in`} style={{animationDelay: `${index * 100}ms`}}>
                  ...
                  <div>
                    <h2 className="font-bold text-xl text-amber-50 group-hover:text-gradientRed transition-colors duration-300">{project.Title}</h2>
                    <p className="text-sm mt-2 text-amber-50 dark:text-gray-300 line-clamp-3 overflow-hidden group-hover:text-gray-200 transition-colors duration-300">{project.Description}</p>
                    <div className="flex flex-wrap gap-2 mt-3">
                      {project.Technologies.map((tech, index) => (
                        <span key={index} className="px-2 py-1 bg-yellow-50 dark:bg-gray-600 rounded-full text-xs hover:bg-gradient-to-r hover:from-gradientRed hover:to-gradientOrange hover:text-amber-50 transition-all duration-300 cursor-pointer">
                          {tech}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex mt-2 justify-end">
                    <button className="flex gap-2 rounded-lg text-amber-50 bg-gray-900 px-8 py-2 w-fit hover:bg-gradient-to-r hover:from-gradientRed hover:to-gradientOrange hover:scale-105 transition-all duration-300 group-hover:shadow-lg group-hover:shadow-gradientRed/30">
            ...
            {Project.length > 6 && (
              <div className="flex justify-start mt-6">
                <button 
                  ...
                  className="px-6 py-2 bg-gradientMaroon dark:bg-gray-700/50 rounded-lg font-semibold hover:bg-gradient-to-r hover:from-gradientRed hover:to-gradientOrange hover:scale-105 hover:shadow-lg transition-all duration-300"
                >
              ...
              {getDisplayData(Certificates, 'certificates').map((certificates, index) => (
                ...
                >
                  ...
                  <div className="items-center absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/70 to-black/40 p-3 group-hover:from-gradientRed/90 group-hover:via-gradientMaroon/70 group-hover:to-gradientOrange/40 transition-all duration-500">
                    <h2 className="text-amber-50 text-lg font-semibold group-hover:text-yellow-100 transition-colors duration-300">{certificates.Title}</h2>
            ...
            {Certificates.length > 6 && (
              <div className="flex justify-start mt-6">
                <button 
                  ...
                  className="px-6 py-2 bg-gradientMaroon dark:bg-gray-700/50 rounded-lg font-semibold hover:bg-gradient-to-r hover:from-gradientMaroon hover:to-gradientOrange hover:scale-105 hover:shadow-lg transition-all duration-300"
                >
        ...
        {activeTab === 'techstack' && (
          <div className="flex flex-wrap justify-center gap-6 w-full max-w-7xl mx-auto animate-slide-in-up delay-300">
            {TechStack.map((techstack, index) => (
              <div 
                key={techstack.id} 
                className={`group flex flex-col p-6 gap-3 bg-gradientMaroon text-amber-50 dark:bg-gray-700/50 rounded-xl items-center w-48 hover:scale-110 hover:bg-gray-600/70 hover:shadow-2xl hover:shadow-gradientOrange/20 transition-all duration-500 cursor-pointer animate-fade-in`}
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4" onClick={closeCertificateModal}>
          <div className="relative max-w-6xl w-full max-h-[95vh] rounded-lg overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              ...
              className="absolute top-4 right-4 z-10 bg-black/60 hover:bg-black/80 text-amber-50 rounded-full p-3 transition-all duration-300 backdrop-blur-sm hover:scale-110"
            >
